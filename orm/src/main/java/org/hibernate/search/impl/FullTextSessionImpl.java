...
public class FullTextSessionImpl extends SessionDelegatorBaseImpl implements FullTextSession, SessionImplementor {
	...
	public FullTextSessionImpl(Session session) {
		//we need to verify nullity before invoking the parent ctor to customize the error message
		super( (SessionImplementor) session, notNull( session ) );
		this.transactionContext = new EventSourceTransactionContext( (EventSource) session );
	}

	private static Session notNull(Session session) {
		if ( session == null ) {
